...
import { JOURNAL_POSTS, IMAGES } from '../constants';
...
const JournalPage = () => (
  <div>
    ...
    <section className="container-kn mb-16 md:mb-24">
      <Link to={JOURNAL_POSTS[0].href} className="group grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 items-center">
        <LazyImage
        ...
        >
          <div className="flex items-center gap-4 text-sm text-text-muted">
            <span className="label-caps">{JOURNAL_POSTS[0].category}</span>
            <span>{JOURNAL_POSTS[0].date}</span>
          ...
          <p className="body-lg text-text-muted">{JOURNAL_POSTS[0].excerpt}</p>
    ...
    <section className="container-kn section-gap">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-12">
        {JOURNAL_POSTS.slice(1).map((post, index) => (
          <motion.article
            key={post.id}
